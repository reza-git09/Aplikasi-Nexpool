...
class _HomePageState extends State<HomePage> {
  ...
  // ============================================================
  // DATA KOLAM CADANGAN
  // Digunakan untuk gambar, rating, harga, jam, dll yang belum tersedia dari API.
  // ============================================================
  ...
  Future<void> _loadPools() async {
    try {
      ...
      final List<dynamic> data = await ApiService.getKolamRenang();
      ...
      // Jika API berhasil tetapi data kosong, gunakan data lokal.
      if (data.isEmpty) {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        // Ambil data lokal berdasarkan urutan. Data lokal dipakai untuk data yang belum tersedia di API.
        final Map<String, dynamic> local = pools[i < pools.length ? i : 0];

        // NAMA KOLAM DARI API
        final String namaKolam =
            item['nama_kolam'] != null && item['nama_kolam'].toString().trim().isNotEmpty
                ? item['nama_kolam'].toString().trim()
                : local['name'].toString();

        // POOL ID
        final String poolId = item['pool_id']?.toString() ?? '';

        // LOKASI
        String location = local['location']?.toString() ?? 'Kabupaten Jember';
        final String alamat = item['alamat']?.toString() ?? '';
        final String kota = item['kota']?.toString() ?? '';
        ...
        // DESKRIPSI
        String sub = local['sub']?.toString() ?? 'Wisata kolam renang';
        ...
        // GAMBAR
        String image = local['image']?.toString() ?? '';
        final String gambar = item['gambar']?.toString() ?? '';

        // Jika API memiliki gambar, gunakan gambar dari API. Jika kosong/null, gunakan asset lokal.
        if (gambar.isNotEmpty && gambar != 'null') {
          image = gambar;
        }
        ...
        debugPrint('Kolam: $namaKolam | Pool ID: $poolId');
      }
      ...
      setState(() {
        pools = apiPools;
        // Jika pilihan kolam tidak valid, kembali ke Semua Kolam.
      ...
      debugPrint('====================================');
      debugPrint('✅ API BERHASIL: ${apiPools.length} kolam diterima.');
      debugPrint('====================================');
    } catch (e) {
      // Jika API gagal, data lokal tetap digunakan.
      debugPrint('====================================');
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic>? selected =
        allPool || selectedPool < 1 || selectedPool > pools.length ? null : pools[selectedPool - 1];

    final Map<String, dynamic> detailPool = selected ?? pools[0];
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildPoolSelector(),
              if (!allPool) _buildSelectedPool(selected!),
              const SizedBox(height: 22),
              _buildTitle('Menu Cepat', Icons.apps_rounded, const Color(0xff123C73)),
  ...
  Widget _buildHero(Map<String, dynamic>? pool) {
    ...
    final String image =
        allPool ? 'assets/images/hero_banner.png' : (pool['image']?.toString() ?? 'assets/images/hero_banner.png');

    final String title = allPool ? 'Selamat Datang di\nNexPool! 🏊' : (pool['fullName']?.toString() ?? 'NEXPOOL');

    final String subtitle =
        allPool ? 'Pilih kolam renang favoritmu di bawah' : (pool['sub']?.toString() ?? 'Informasi kolam renang NEXPOOL');

    final String badge = allPool
        ? '🌊 ${pools.length} Wisata Kolam Renang Jember'
        : '⭐ Rating ${pool['rating']?.toString() ?? '-'} · ${pool['reviews']?.toString() ?? '0'} Ulasan';

    return SizedBox(
      height: 285,
      child: Stack(
        children: [
          Positioned.fill(child: _buildImage(image, fit: BoxFit.cover)),
          Positioned.fill(
            child: Container(
              decoration: const BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: [Color.fromARGB(50, 0, 0, 0), Color.fromARGB(220, 0, 45, 65)],
                ),
              ),
            ),
          ),
              ...
              children: [
                Container(
                  ...
                  child: Image.asset(
                    ...
                    fit: BoxFit.contain,
                    errorBuilder: (_, __, ___) => const Icon(Icons.pool, color: Colors.white),
                  ),
                ),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Text(badge, style: const TextStyle(color: Color(0xffFFD166), fontSize: 10, fontWeight: FontWeight.w800)),
                ),
                ...
                Text(
                  ...
                  style: const TextStyle(color: Colors.white, fontSize: 25, height: 1.15, fontWeight: FontWeight.w900),
                ),
  ...
  // IMAGE HELPER
  // ============================================================
  Widget _buildImage(String image, {BoxFit fit = BoxFit.cover}) {
    if (image.startsWith('http://') || image.startsWith('https://')) {
      return Image.network(image, fit: fit, errorBuilder: (_, __, ___) => _imageFallback());
    }
    return Image.asset(image, fit: fit, errorBuilder: (_, __, ___) => _imageFallback());
  }

  Widget _imageFallback() {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(colors: [Color(0xff0077A8), Color(0xff00B4D8)]),
      ),
      child: const Center(child: Icon(Icons.pool, size: 70, color: Colors.white)),
    );
  }
  ...
  Widget _buildPoolSelector() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildTitle('Pilih Kolam Renang', Icons.pool, const Color(0xff00A4C6)),
        const SizedBox(height: 5),
        const Padding(
          padding: EdgeInsets.symmetric(horizontal: 20),
          child: Text('Pilih destinasi untuk melihat informasi lengkap', style: TextStyle(color: Colors.grey, fontSize: 11)),
        ),
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              if (index == 0) return _allPoolCard();
              return _poolCard(index, pools[index - 1]);
            },
          ),
        ),
      ],
  ...
  // ============================================================
  Widget _allPoolCard() {
    final bool selected = selectedPool == 0;
  ...
  // POOL CARD
  // ============================================================
  Widget _poolCard(int index, Map<String, dynamic> pool) {
    final bool selected = selectedPool == index;
    final String image = pool['image']?.toString() ?? '';
    final String name = pool['name']?.toString() ?? 'Kolam Renang';
    final String rating = pool['rating']?.toString() ?? '-';
  ...
  // SELECTED POOL
  // ============================================================
  Widget _buildSelectedPool(Map<String, dynamic> pool) {
    final String fullName = pool['fullName']?.toString() ?? 'Kolam Renang';
    final String sub = pool['sub']?.toString() ?? 'Wisata kolam renang';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xff0077A8), Color(0xff00B4D8)]),
      ...
      child: Row(
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  maxLines: 2,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(color: Colors.white.withOpacity(0.78), fontSize: 9.5),
                ),
              ],
            ),
          ),
          GestureDetector(
            ...
            child: const Text('← Semua', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
    ...
    final List<Map<String, dynamic>> promos = [
      {
        ...
        'place': 'Tiara Jember Park Waterboom · Sep 2026',
        'discount': 'Hemat 30%',
      ...
      },
      {
        ...
        'place': 'Pemandian Kebon Agung · Senin–Rabu',
        'discount': 'Gratis 1',
      ...
      },
      {
        ...
        'place': 'Annasya Waterpark · Sabtu–Minggu',
        'discount': 'Diskon 25%',
      ...
      },
      {
        ...
        'place': 'Jati Park · Sebelum pukul 09.00',
        'discount': 'Hemat 15%',
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final String title = promo['title']?.toString() ?? 'Promo';
              final String place = promo['place']?.toString() ?? '';
              final String discount = promo['discount']?.toString() ?? '';
              final Color color1 = promo['color1'] is Color ? promo['color1'] as Color : const Color(0xff0077A8);
              final Color color2 = promo['color2'] is Color ? promo['color2'] as Color : const Color(0xff00B4D8);

              return GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PromoPage())),
                child: Container(
                  width: 250,
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            discount,
                            style: const TextStyle(color: Color(0xff5A3A00), fontSize: 9, fontWeight: FontWeight.w900),
                          ),
                        ),
                      ),
                    ],
    ...
    final List<Map<String, String>> events = [
      {
        ...
        'desc': 'Lomba berenang berhadiah jutaan rupiah',
        'badge': 'Segera',
      },
      {
        'date': '25 Sep 2026',
        ...
        'desc': 'Senam air & games seru untuk si kecil',
        'badge': 'Gratis',
      },
      {
        'date': '1 Okt 2026',
        ...
        'desc': 'Renang malam dengan live music & DJ',
        'badge': 'Hot 🔥',
      },
      {
        'date': '5 Okt 2026',
        ...
        'desc': 'Hunting foto di taman air alami',
        'badge': 'Hadiah',
      },
      {
        'date': '10 Okt 2026',
        ...
        'desc': 'Kompetisi renang antar sekolah Jember',
        'badge': 'Daftar',
      },
    ];
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  padding: const EdgeInsets.all(15),
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [Color(0xff023E58), Color(0xff0077A8)]),
                    borderRadius: BorderRadius.circular(18),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: Text(
                              ...
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: TextStyle(color: Colors.white.withOpacity(0.72), fontSize: 8.5, fontWeight: FontWeight.w700),
                            ),
                          ),
                          Container(
                      ...
                      const SizedBox(height: 6),
                      Text(event['title'] ?? '-', style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 3),
                      Text(
                        event['desc'] ?? '-',
                        maxLines: 2,
  ...
  void _eventMessage() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Halaman Event akan kita buat berikutnya.')),
    );
  }
  ...
  // HARGA TIKET
  // ============================================================
  Widget _buildPriceSection(Map<String, dynamic> pool) {
    final String weekday = pool['weekday']?.toString() ?? 'Rp0';
    final String weekend = pool['weekend']?.toString() ?? 'Rp0';
    final String name = pool['fullName']?.toString() ?? 'Kolam Renang';
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [Color(0xff0077A8), Color(0xff023E58)]),
            borderRadius: BorderRadius.circular(18),
          ),
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: Column(
                      children: [
                        Text('Weekday', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 10)),
                        const SizedBox(height: 4),
                        Text(weekday, style: const TextStyle(color: Color(0xffFFD166), fontSize: 19, fontWeight: FontWeight.w900)),
                        const SizedBox(height: 2),
                        const Text('Senin – Jumat', style: TextStyle(color: Colors.white70, fontSize: 8)),
                      ],
                    ),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      children: [
                        Text('Weekend', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 10)),
                        const SizedBox(height: 4),
                        Text(weekend, style: const TextStyle(color: Color(0xffFFD166), fontSize: 19, fontWeight: FontWeight.w900)),
                        const SizedBox(height: 2),
                        const Text('Sabtu – Minggu', style: TextStyle(color: Colors.white70, fontSize: 8)),
                      ],
                    ),
                  ),
                ],
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final String name = item['name']?.toString() ?? 'Fasilitas';
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Sedang Buka', style: TextStyle(color: Color(0xff06A87E), fontSize: 10, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 3),
                    Text(hours, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 2),
                    const Text('Buka setiap hari', style: TextStyle(color: Colors.grey, fontSize: 9)),
                  ],
                ),
              ),
              GestureDetector(
  ...
  // RATING
  // ============================================================
  Widget _buildRatingSection(Map<String, dynamic> pool) {
    final String rating = pool['rating']?.toString() ?? '0.0';
    final String ratingCount = pool['ratingCount']?.toString() ?? '0 ulasan';

    double bar5 = 0.8;
    if (pool['bar5'] is num) {
      bar5 = (pool['bar5'] as num).toDouble();
    }
          ...
          child: Row(
            children: [
              SizedBox(
                ...
                child: Column(
                  children: [
                    Text(rating, style: const TextStyle(fontSize: 37, fontWeight: FontWeight.w900)),
                    const Text('★★★★★', style: TextStyle(color: Color(0xffFFD166), fontSize: 14, letterSpacing: 1)),
                    const SizedBox(height: 3),
                    Text(ratingCount, style: const TextStyle(color: Colors.grey, fontSize: 9)),
                  ],
                ),
              ),
  ...
  Widget _ratingBar(String number, double value) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(width: 10, child: Text(number, style: const TextStyle(color: Colors.grey, fontSize: 9))),
          ...
          SizedBox(
            width: 28,
            child: Text('${(safeValue * 100).round()}%', textAlign: TextAlign.right, style: const TextStyle(color: Colors.grey, fontSize: 8)),
          ),
        ],
      ),
    );
  ...
  // TITLE
  // ============================================================
  Widget _buildTitle(String title, IconData icon, Color color, {String? action, VoidCallback? onAction}) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 20),
      child: Row(
        children: [
          ...
          const SizedBox(width: 5),
          Expanded(
            child: Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: Color(0xff172B4D))),
          ),
          if (action != null)
            GestureDetector(
              onTap: onAction,
              child: Text(action, style: const TextStyle(color: Color(0xff00A4C6), fontSize: 10, fontWeight: FontWeight.w800)),
            ),
        ],
      ),
    );
